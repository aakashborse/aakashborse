...
Phase C additions:
  - TOU-aware energy billing (GridTariffModel.tou_blocks)
  - Monthly peak demand charges (GridTariffModel.demand_charge)
  - demand_charge_cost field in YearCashflow
"""
...
class YearCashflow:
    ...
    demand_charge_cost: float = 0.0  # NEW: kW-based demand charges
...
class FinanceResult:
    ...
    total_demand_charge_cost: float = 0.0  # NEW
...
# Approximate hours per calendar month (for demand charge grouping)
...
class FinanceEngine:
    ...
    def run(self) -> FinanceResult:
        ...
        for yr in range(1, self.n_years + 1):
            ...
            # --- Grid energy cost (TOU-aware) ---
            cf.grid_energy_cost = self._compute_tou_energy_cost(sim, escalation)
            cf.grid_energy_cost += self.cfg.grid.fixed_charge_rs_month * 12

            # --- Demand charge (kW-based) ---
            cf.demand_charge_cost = self._compute_demand_charge_cost(sim, escalation)
        ...
        fr.total_demand_charge_cost = sum(cf.demand_charge_cost for cf in fr.cashflows)
        fr.total_cost_with_outages = sum(
            cf.capex + cf.replacement_cost + cf.total_opex for cf in fr.cashflows
        )
    ...
    # ------------------------------------------------------------------
    # TOU-aware energy cost
    # ------------------------------------------------------------------

    def _compute_tou_energy_cost(self, sim: SimulationResult, escalation: float) -> float:
        """
        Compute grid energy cost using TOU rates if configured,
        falling back to flat rate.

        Iterates hourly results to apply the correct rate per time-of-day.
        """
        grid_tariff = self.cfg.grid

        if not grid_tariff.tou_blocks:
            # Flat rate — same as before, but we use rate_for_hour for consistency
            return sim.total_grid_kwh * grid_tariff.energy_rate_rs_kwh * escalation

        # TOU billing: sum rate × kWh for each hour
        total_cost = 0.0
        for h, hr in enumerate(sim.hourly):
            grid_kwh = hr.grid_to_load + hr.grid_to_bess
            if grid_kwh > 0:
                rate = grid_tariff.rate_for_hour(h)
                total_cost += grid_kwh * rate

        return total_cost * escalation

    # ------------------------------------------------------------------
    # Demand charge (monthly peak kW-based)
    # ------------------------------------------------------------------

    def _compute_demand_charge_cost(self, sim: SimulationResult, escalation: float) -> float:
        """
        Compute annual demand charge cost from monthly peak grid draws.

        Algorithm:
        1. Split 8760 hourly results into 12 calendar months.
        2. Find peak grid draw (kW) for each month.
        3. Apply demand charge rate (₹/kW/month).
        4. Apply ratchet clause if configured: min demand = ratchet_pct × annual_peak.
        5. Apply contracted demand floor if configured.
        6. Escalate at grid tariff escalation rate.
        """
        dc_model = self.cfg.grid.demand_charge
        if dc_model is None:
            return 0.0

        # --- Split hourly data into months ---
        monthly_peaks: list[float] = []
        h = 0
        annual_peak_kw = 0.0

        for hrs in _HOURS_PER_MONTH:
            month_hours = sim.hourly[h : h + hrs]
            if not month_hours:
                monthly_peaks.append(0.0)
            else:
                # Peak grid demand this month (kW; for 1-h slots kW == kWh)
                peak = max(hr.grid_to_load + hr.grid_to_bess for hr in month_hours)
                monthly_peaks.append(peak)
                annual_peak_kw = max(annual_peak_kw, peak)
            h += hrs

        # --- Apply demand charge per month ---
        annual_cost = 0.0
        for month_peak in monthly_peaks:
            billable = month_peak

            # Ratchet clause: minimum billable demand = ratchet_pct % of annual peak
            if dc_model.ratchet_pct is not None:
                ratchet_floor = dc_model.ratchet_pct / 100.0 * annual_peak_kw
                billable = max(billable, ratchet_floor)

            # Contracted demand floor
            if dc_model.contracted_demand_kw is not None:
                billable = max(billable, dc_model.contracted_demand_kw)

            annual_cost += dc_model.charge_rs_kw_month * billable

        return annual_cost * escalation
